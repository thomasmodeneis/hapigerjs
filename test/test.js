...
describe('Testing HapiGER events', function () {
    ...
    it('Driver should recommend item to person ', function () {
        var p = client.POST("/recommendations", {
                "namespace": "test",
                "person": "Thomas",
                "configuration": {
        ...
        return expect(p).to.eventually.have.property('recommendations');
    });

    it('Driver should compact a namespace', function () {
        var p = client.POST("/compact", {
            "namespace": "test"});
        return expect(p).to.eventually.have.property('init_count');
    });

    it('Driver should delete a namespace', function () {
        var p = client.DELETE("/namespaces/test");
        return expect(p).to.eventually.have.property('namespace');
    });
